...
function DisplayOrdersPage(req, res, next) {
  orderData_1.default.find((err, OrderDataCollection) => {
    if (err){
      console.error(err);
      res.end(err);
    }
    res.render('index', {title: 'Your Orders', page: 'orders', OrderData: OrderDataCollection, displayName: Util_1.UserDisplayName(req), emailAddress: Util_1.UserEMail(req), });
  });
}
