...
function DisplayOrdersPage(req, res, next) {
  orderData_1.default.find((err, orderData) => {
    if (err){
      console.error(err);
      res.end(err);
    }
    res.render('index', {title: 'Your Orders', page: 'orders', OrderDataCollection: orderData, displayName: Util_1.UserDisplayName(req), emailAddress: Util_1.UserEMail(req), });
  });
}
